import { motion } from "framer-motion";
import { section } from "../animations/animation-section";
import { data_projects, data_projects2 } from "../data/projects_data";
import { projectOpacityAnimation } from "../animations/animation-project-honey";
import { projectsAnimation, moreProjectsAnimation } from "../animations/animation-projects";
import { useState } from "react";
import { useEffect } from "react";


function Projects() {
    const [bool, setBool] = useState(true);
    const [button, setButton] = useState(false);


    function style() {
        setBool(prev => !prev)
    }

    useEffect(() => {
        if (bool) {
            document.getElementById('gallery-body').style.display = 'flex';
            document.getElementById('second-style').style.display = 'none';
        } else {
            document.getElementById('gallery-body').style.display = 'none';
            document.getElementById('second-style').style.display = 'grid';
        }
    }, [bool])

    return (
        <motion.div initial='hidden' whileInView='visible' viewport={{ once: true }} id="Projects" className='projects sectionDiv'>
            <motion.h1 className="section_name" variants={section}>Projects</motion.h1>
            <button onClick={style}>change style</button>

            <div id="gallery-body" className="gallery-body">
                <div className="gallery">
                    {data_projects.map((item, index) => <motion.div initial='hidden' whileInView='visible' viewport={{ once: true }} custom={index + 1} variants={projectOpacityAnimation} className="project_item" key={index} style={{ background: `url(${item.img})` }} onMouseOver={() => document.getElementById(`project-${index}`).classList.remove('hide')} onMouseOut={() => document.getElementById(`project-${index}`).classList.add('hide')}>
                        <div id={`project-${index}`} className="project_info hide">
                            <ul className="project_technologies">
                                {item.svg.map((icon, index) => <li key={index}> {icon} </li>)}
                            </ul>
                            <div className="project_name">
                                <a title="Click to view" href={item.url} target="_blank" rel="noreferrer">{item.name}</a>
                            </div>
                            <div title="End time" className="project_date">
                                {item.date}
                            </div>
                            <div className="github_link">
                                <a title="GitHub" href={item.github} target="_blank" rel="noreferrer"><svg className="github_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px">    <path d="M17.791,46.836C18.502,46.53,19,45.823,19,45v-5.4c0-0.197,0.016-0.402,0.041-0.61C19.027,38.994,19.014,38.997,19,39 c0,0-3,0-3.6,0c-1.5,0-2.8-0.6-3.4-1.8c-0.7-1.3-1-3.5-2.8-4.7C8.9,32.3,9.1,32,9.7,32c0.6,0.1,1.9,0.9,2.7,2c0.9,1.1,1.8,2,3.4,2 c2.487,0,3.82-0.125,4.622-0.555C21.356,34.056,22.649,33,24,33v-0.025c-5.668-0.182-9.289-2.066-10.975-4.975 c-3.665,0.042-6.856,0.405-8.677,0.707c-0.058-0.327-0.108-0.656-0.151-0.987c1.797-0.296,4.843-0.647,8.345-0.714 c-0.112-0.276-0.209-0.559-0.291-0.849c-3.511-0.178-6.541-0.039-8.187,0.097c-0.02-0.332-0.047-0.663-0.051-0.999 c1.649-0.135,4.597-0.27,8.018-0.111c-0.079-0.5-0.13-1.011-0.13-1.543c0-1.7,0.6-3.5,1.7-5c-0.5-1.7-1.2-5.3,0.2-6.6 c2.7,0,4.6,1.3,5.5,2.1C21,13.4,22.9,13,25,13s4,0.4,5.6,1.1c0.9-0.8,2.8-2.1,5.5-2.1c1.5,1.4,0.7,5,0.2,6.6c1.1,1.5,1.7,3.2,1.6,5 c0,0.484-0.045,0.951-0.11,1.409c3.499-0.172,6.527-0.034,8.204,0.102c-0.002,0.337-0.033,0.666-0.051,0.999 c-1.671-0.138-4.775-0.28-8.359-0.089c-0.089,0.336-0.197,0.663-0.325,0.98c3.546,0.046,6.665,0.389,8.548,0.689 c-0.043,0.332-0.093,0.661-0.151,0.987c-1.912-0.306-5.171-0.664-8.879-0.682C35.112,30.873,31.557,32.75,26,32.969V33 c2.6,0,5,3.9,5,6.6V45c0,0.823,0.498,1.53,1.209,1.836C41.37,43.804,48,35.164,48,25C48,12.318,37.683,2,25,2S2,12.318,2,25 C2,35.164,8.63,43.804,17.791,46.836z" /></svg></a>
                            </div>
                        </div>
                    </motion.div>)}
                </div>
            </div>

            <motion.div initial="hidden" whileInView='visible' viewport={{ once: true }} id="second-style">
                {data_projects.map((item, index) => <motion.div custom={index + 1} variants={projectsAnimation} key={index} className={`project-${index + 1}-new project_item_new`} style={{ background: `url(${item.img2})` }} onMouseOver={() => document.getElementById(`project-${index}-new`).classList.remove('hide')} onMouseOut={() => document.getElementById(`project-${index}-new`).classList.add('hide')}>
                    <div id={`project-${index}-new`} className="project_info hide">
                        <div>
                            <ul className="project_technologies">
                                {item.svg.map((icon, index) => <li key={index}> {icon} </li>)}
                            </ul>
                            <div className="project_name">
                                <a title="Click to view" href={item.url} target="_blank" rel="noreferrer">{item.name}</a>
                            </div>
                            <div title="End time" className="project_date">
                                {item.date}
                            </div>
                            <div className="github_link">
                                <a title="GitHub" href={item.github} target="_blank" rel="noreferrer"><svg className="github_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px">    <path d="M17.791,46.836C18.502,46.53,19,45.823,19,45v-5.4c0-0.197,0.016-0.402,0.041-0.61C19.027,38.994,19.014,38.997,19,39 c0,0-3,0-3.6,0c-1.5,0-2.8-0.6-3.4-1.8c-0.7-1.3-1-3.5-2.8-4.7C8.9,32.3,9.1,32,9.7,32c0.6,0.1,1.9,0.9,2.7,2c0.9,1.1,1.8,2,3.4,2 c2.487,0,3.82-0.125,4.622-0.555C21.356,34.056,22.649,33,24,33v-0.025c-5.668-0.182-9.289-2.066-10.975-4.975 c-3.665,0.042-6.856,0.405-8.677,0.707c-0.058-0.327-0.108-0.656-0.151-0.987c1.797-0.296,4.843-0.647,8.345-0.714 c-0.112-0.276-0.209-0.559-0.291-0.849c-3.511-0.178-6.541-0.039-8.187,0.097c-0.02-0.332-0.047-0.663-0.051-0.999 c1.649-0.135,4.597-0.27,8.018-0.111c-0.079-0.5-0.13-1.011-0.13-1.543c0-1.7,0.6-3.5,1.7-5c-0.5-1.7-1.2-5.3,0.2-6.6 c2.7,0,4.6,1.3,5.5,2.1C21,13.4,22.9,13,25,13s4,0.4,5.6,1.1c0.9-0.8,2.8-2.1,5.5-2.1c1.5,1.4,0.7,5,0.2,6.6c1.1,1.5,1.7,3.2,1.6,5 c0,0.484-0.045,0.951-0.11,1.409c3.499-0.172,6.527-0.034,8.204,0.102c-0.002,0.337-0.033,0.666-0.051,0.999 c-1.671-0.138-4.775-0.28-8.359-0.089c-0.089,0.336-0.197,0.663-0.325,0.98c3.546,0.046,6.665,0.389,8.548,0.689 c-0.043,0.332-0.093,0.661-0.151,0.987c-1.912-0.306-5.171-0.664-8.879-0.682C35.112,30.873,31.557,32.75,26,32.969V33 c2.6,0,5,3.9,5,6.6V45c0,0.823,0.498,1.53,1.209,1.836C41.37,43.804,48,35.164,48,25C48,12.318,37.683,2,25,2S2,12.318,2,25 C2,35.164,8.63,43.804,17.791,46.836z" /></svg></a>
                            </div>
                        </div>

                    </div>
                </motion.div>)}
            </motion.div>

            <div className="more_projects">
                <button className="projects_button" onClick={() => setButton(prev => !prev)}>{!button ? 'show more' : 'show less'}</button>
                {button && <motion.ul initial="hidden" whileInView='visible' viewport={{ once: true }}>
                    {data_projects2.map((item, index) => <motion.li variants={moreProjectsAnimation} custom={index} key={index}>
                        <a title="Click to view" href={item.url} target="_blank" rel="noreferrer">{item.name}</a>
                        <p title="End time">{item.date}</p>
                        {Object.keys(item)[Object.keys(item).length - 1] === 'github' && <a title="GitHub" className="more_projects_icon" href={item.github} target="_blank" rel="noreferrer" ><svg style={{ width: 30, height: 30, fill: '#dd8109' }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px">    <path d="M17.791,46.836C18.502,46.53,19,45.823,19,45v-5.4c0-0.197,0.016-0.402,0.041-0.61C19.027,38.994,19.014,38.997,19,39 c0,0-3,0-3.6,0c-1.5,0-2.8-0.6-3.4-1.8c-0.7-1.3-1-3.5-2.8-4.7C8.9,32.3,9.1,32,9.7,32c0.6,0.1,1.9,0.9,2.7,2c0.9,1.1,1.8,2,3.4,2 c2.487,0,3.82-0.125,4.622-0.555C21.356,34.056,22.649,33,24,33v-0.025c-5.668-0.182-9.289-2.066-10.975-4.975 c-3.665,0.042-6.856,0.405-8.677,0.707c-0.058-0.327-0.108-0.656-0.151-0.987c1.797-0.296,4.843-0.647,8.345-0.714 c-0.112-0.276-0.209-0.559-0.291-0.849c-3.511-0.178-6.541-0.039-8.187,0.097c-0.02-0.332-0.047-0.663-0.051-0.999 c1.649-0.135,4.597-0.27,8.018-0.111c-0.079-0.5-0.13-1.011-0.13-1.543c0-1.7,0.6-3.5,1.7-5c-0.5-1.7-1.2-5.3,0.2-6.6 c2.7,0,4.6,1.3,5.5,2.1C21,13.4,22.9,13,25,13s4,0.4,5.6,1.1c0.9-0.8,2.8-2.1,5.5-2.1c1.5,1.4,0.7,5,0.2,6.6c1.1,1.5,1.7,3.2,1.6,5 c0,0.484-0.045,0.951-0.11,1.409c3.499-0.172,6.527-0.034,8.204,0.102c-0.002,0.337-0.033,0.666-0.051,0.999 c-1.671-0.138-4.775-0.28-8.359-0.089c-0.089,0.336-0.197,0.663-0.325,0.98c3.546,0.046,6.665,0.389,8.548,0.689 c-0.043,0.332-0.093,0.661-0.151,0.987c-1.912-0.306-5.171-0.664-8.879-0.682C35.112,30.873,31.557,32.75,26,32.969V33 c2.6,0,5,3.9,5,6.6V45c0,0.823,0.498,1.53,1.209,1.836C41.37,43.804,48,35.164,48,25C48,12.318,37.683,2,25,2S2,12.318,2,25 C2,35.164,8.63,43.804,17.791,46.836z" /></svg></a>}
                    </motion.li>
                    )}
                </motion.ul>}
            </div>

        </motion.div>
    )
}

export default Projects;